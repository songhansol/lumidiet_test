...
import com.doubleh.lumidiet.pedometer.util.Logger;
...
public class SensorListenerService extends Service implements SensorEventListener {
    ...
    private static final String TAG = SensorListenerService.class.getSimpleName();
    ...
    public void onCreate() {
        super.onCreate();
        if (BuildConfig.DEBUG) Logger.log(TAG + " onCreate");
        reRegisterSensor();
        updateNotificationState();
    }
    ...
    public void onTaskRemoved(final Intent rootIntent) {
        ...
        Crashlytics.log("sensor service task removed");
        Answers.getInstance().logCustom(new CustomEvent("sensor service task removed"));
    ...
    public void onDestroy() {
        super.onDestroy();
        if (BuildConfig.DEBUG) Logger.log(TAG + " onDestroy");
        Crashlytics.log(TAG + " onDestroy");
        Answers.getInstance().logCustom(new CustomEvent(TAG + " onDestroy"));
        try {
    ...
    private void updateNotificationState() {
        if (BuildConfig.DEBUG) Logger.log(TAG + " updateNotificationState");
        SharedPreferences prefs = getSharedPreferences("pedometer", Context.MODE_PRIVATE);
        ...
        if (prefs.getBoolean("notification", false)) {
            ...
            if (steps > 0) {
                ...
                notificationBuilder.setContentText(getString(R.string.notification_text, NumberFormat.getInstance(Locale.getDefault())
...
//                                    PendingIntent.FLAG_UPDATE_CURRENT)).setOngoing(true);
            ...
            if (steps > 0) {
                if (today_offset == Integer.MIN_VALUE) today_offset = -steps;
                Logger.log("Today count:" + (today_offset + steps));
            }
        }
    }
