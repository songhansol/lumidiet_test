 ...
 *
 * @customizing Byun
 * @day 1.3.1 (08. 26. 2016)
 * Add Start Progress Setting
 * Add ImageView(AnimationDrawable)
 */
public class HoloCircularProgressBar extends View {
    ...
    public void setThumbImageView(ImageView imageView) {
        ...
        /*for (int i = 0; i < 30; i++)
        {
            String imgPath = String.format("circle_ani/main_circle_ani%d.png", i + 1);
            BitmapDrawable frame = (BitmapDrawable)AssetsReader.getDrawableFromAssets(getContext(), imgPath);
            mAnimationDrawable.addFrame(frame, 100);
        }*/

        //mImageView.setBackground(mAnimationDrawable);

        mImageView.setBackgroundResource(R.drawable.circle_animation_list);

        float minusX = mImageView.getWidth() / 2;
        mImageView.setTranslationX(mImageView.getX() - minusX);

        mAnimationDrawable = (AnimationDrawable) mImageView.getBackground();
    }

    /**
     * Set Thumb Image View Visibility
     * @param visible
     */
    public void imageViewVisible(boolean visible) {
    ...
    public void showImageView() {
        ...
        {
            if (BuildConfig.DEBUG) Log.d(TAG, "AnimationDrawable is null");
            return;
        }
    ...
    /**
     * Set Relative Layout (on Thumb ImageView)
     * @param layout
     */
    public void setThumbImageLayout(RelativeLayout layout) { mImageViewLayout = layout; }
    ...
    /**
     * Is Set Start Progress Zero? y: true, n: false
     */
    private boolean isStartProgressZero = true;
    ...
    public void setStartProgress(float progress) {
        if (progress > 0.0f)
            isStartProgressZero = false;
        else
            isStartProgressZero = true;
        mProgress = progress;
    }

    /**
     * if Start Progress != 0, then Draw progress 0 to start progress
     */
     ...
     * @param value
     */
    //public void setIsFirstDraw(boolean value) { isFirstDraw = value; }

    ////////////////////

    public void setCircleStrokeWidth(int strokeWidth) {
        mCircleStrokeWidth = strokeWidth;
        mThumbRadius = (int) (mCircleStrokeWidth * 2.3);
    }
    ...
    protected void onDraw(final Canvas canvas) {
        ...
        if (!isStartProgressZero)
        {
            // if Start Progress not zero, then Draw progress 0 to start degree
            // and draw start progress
            if (!isFirstDraw) {
                //Log.d(TAG, "Just one");
                canvas.drawArc(mCircleBounds, 270, 360 * getProgress(), false, mProgressColorPaint);
                isFirstDraw = true;
            }
        ...
        if (isThumbEnabled()) {
            ...
            mImageViewLayout.setRotation(progressRotation);
        }
    }
